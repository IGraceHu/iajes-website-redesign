                ...
                <div className="p-3 flex items-start justify-between">
                    ...
                    {editable ? (
                        <div className="flex space-x-2">
                            <button onClick={(e) => { e.preventDefault(); onEdit && onEdit(article); }} className="p-2 text-gray-600 hover:text-gray-dark" title="Edit" aria-label={`Edit ${article?.title || "article"}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /></svg>
                            </button>
                            <button onClick={(e) => { e.preventDefault(); onDownload && onDownload(article); }} className="p-2 text-gray-600 hover:text-gray-dark" title="Download" aria-label={`Download ${article?.title || "article"}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M3 14a1 1 0 011-1h12a1 1 0 011 1v2a2 2 0 01-2 2H5a2 2 0 01-2-2v-2z" /><path d="M7 10l3 3 3-3M10 2v11" /></svg>
                            </button>
                            <button onClick={(e) => { e.preventDefault(); onDelete && onDelete(article); }} className="p-2 text-red-600 hover:text-red-800" title="Delete" aria-label={`Delete ${article?.title || "article"}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H3a1 1 0 100 2h14a1 1 0 100-2h-2V3a1 1 0 00-1-1H6zM7 8a1 1 0 00-1 1v6a1 1 0 001 1h6a1 1 0 001-1V9a1 1 0 00-1-1H7z" clipRule="evenodd" /></svg>
                            </button>
                        </div>
                    ) : null}
                </div>
