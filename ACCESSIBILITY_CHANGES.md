...
This pass focused on WCAG 2.1 AA-oriented issues that can be remediated in code across the IAJES site. It is not a formal accessibility certification; the deployed site should still be checked with WAVE, axe DevTools, Lighthouse, WebAIM Contrast Checker, and manual keyboard/zoom testing.
...
- Hid decorative icons inside text buttons so screen readers announce the action once.
...
- Fixed content typos noted during the review:
  ...
  - `Bhubanewar` / `Bhubaneshwar` to `Bhubaneswar`
...
## Live-Site Checks Still Required

Run these on the deployed site after the changes are published:

- WAVE Chrome extension on every main public page.
- axe DevTools on every main public page and at least one admin/editor flow.
- Lighthouse Accessibility audit in Chrome DevTools.
- WebAIM Contrast Checker for footer text, card text, overlay text, and light text over images.
- Manual keyboard pass using Tab, Shift+Tab, Enter, Escape, and Arrow keys.
- 200% browser zoom check on desktop and mobile widths.
- Screen reader spot check for navigation, dropdowns, dialogs, forms, and repeated card links.

Manual testing matters because automated tools cannot fully verify reading order, meaningful alt text quality, color contrast over dynamic images, or whether tab order feels logical in real use.
